...
先得到目标架构的 `rust-yu-tauri.exe`，再运行：

```powershell
.\tools\release\package-portable.ps1 `
  -BinaryPath .\src-tauri\target\release\rust-yu-tauri.exe
```
...
- 当前 Parallels Windows ARM 开发环境的 `aarch64-pc-windows-gnullvm` Tauri 构建可能被 `tauri-winres` 的 `windres` 工具链阻断；这属于构建机问题，不能把核心 Rust/前端通过误报成完整安装包已通过。
