...
use std::env;
#[cfg(all(target_os = "windows", target_arch = "aarch64", target_env = "gnu"))]
...
impl WindowsResource {
    ...
    pub fn compile(&self) -> io::Result<()> {
        ...
        // LLVM-MinGW 的 llvm-windres 尚不接受 pe-aarch64-little；使用
        // llvm-rc + llvm-cvtres 生成带 ARM64 machine type 的 COFF 资源。
        // 其他目标保持上游 embed-resource 路径，避免改变现有发布行为。
        #[cfg(all(target_os = "windows", target_arch = "aarch64", target_env = "gnu"))]
        {
            compile_arm64_resource(&rc, &output)?;
        }

        #[cfg(not(all(target_os = "windows", target_arch = "aarch64", target_env = "gnu")))]
        {
...
#[cfg(all(target_os = "windows", target_arch = "aarch64", target_env = "gnu"))]
fn compile_arm64_resource(resource_file: &Path, output: &Path) -> io::Result<()> {
    let resource_binary = output.join("resource.res");
    let resource_object = output.join("resource.obj");
    ...
    run_arm64_tool(
        ...
        vec![
            OsString::from("/FO"),
            resource_binary.as_os_str().to_owned(),
            resource_file.as_os_str().to_owned(),
        ],
    )?;
    run_arm64_tool(
        "RUST_YU_LLVM_CVTRES",
        ...
        vec![
            ...
            OsString::from(format!("/OUT:{}", resource_object.display())),
            resource_binary.as_os_str().to_owned(),
        ],
    )?;
    // windres also emits a COFF object despite the historical `.a` suffix;
    // pass the ARM64 object directly so lld includes its `.rsrc` sections.
    println!("cargo:rustc-link-arg={}", resource_object.display());
    Ok(())
}

#[cfg(all(target_os = "windows", target_arch = "aarch64", target_env = "gnu"))]
fn run_arm64_tool(variable: &str, default_name: &str, arguments: Vec<OsString>) -> io::Result<()> {
    ...
    let status = Command::new(&executable)
        ...
        .map_err(|error| {
            io::Error::new(
                error.kind(),
                format!("无法运行 {default_name}（{variable} 或 PATH）：{error}"),
            )
        })?;
    if status.success() {
        Ok(())
